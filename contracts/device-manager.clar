...
;; Data structures
(define-map devices
  { owner: principal, device-id: (buff 65) }
  {
    passkey-id: (buff 65),
...
(define-map device-activity
  { device-id: (buff 65) }
  { activity-count: uint, last-action: (string-ascii 100) }
)

;; Read-only functions

;; Get device info
(define-read-only (get-device-info (owner principal) (device-id (buff 65)))
...
(define-read-only (get-account-devices (owner principal))
  (default-to 
    { device-list: (list) }
    (map-get? account-devices { owner: owner })
  )
)

;; Check if device is authorized
(define-read-only (is-device-authorized (owner principal) (device-id (buff 65)))
  (match (map-get? devices { owner: owner, device-id: device-id })
...
(define-read-only (get-device-permission (owner principal) (device-id (buff 65)))
  (match (map-get? devices { owner: owner, device-id: device-id })
...
(define-read-only (count-devices-by-permission (owner principal) (permission (string-ascii 20)))
  (let
    (
      (all-devices (get device-list (get-account-devices owner)))
    )
...
(define-private (has-permission (owner principal) (device-id (buff 65)) (required-permission (string-ascii 20)))
  (match (map-get? devices { owner: owner, device-id: device-id })
    device-data
      (or 
...
(define-public (register-device
  ...
  (let
    ...
    (asserts! (is-none (map-get? devices { owner: owner, device-id: device-id })) ERR-DEVICE-EXISTS)
    ...
    (asserts! (contract-call? .passkey-registry is-passkey-registered owner passkey-id) ERR-NOT-AUTHORIZED)
    
    ;; Store device info
    (map-set devices
      { owner: owner, device-id: device-id }
      {
    ...
    (map-set account-devices
      { owner: owner }
      { device-list: (unwrap-panic (as-max-len? (concat current-devices (list device-id)) u20)) }
    )
    
    ;; Initialize activity tracking
    (map-set device-activity
      { device-id: device-id }
      { activity-count: u0, last-action: "registered" }
    )
...
(define-public (update-device-permission
  ...
  (let
    (
      (owner tx-sender)
      (device-data (unwrap! (map-get? devices { owner: owner, device-id: device-id }) ERR-DEVICE-NOT-FOUND))
    )
...
(define-public (revoke-device (device-id (buff 65)))
  (let
    ...
    (map-set device-activity
      { device-id: device-id }
      (merge 
        (default-to { activity-count: u0, last-action: "" } (map-get? device-activity { device-id: device-id }))
        { last-action: "revoked" }
      )
    )
...
(define-public (log-device-activity
  ...
  (let
    (
      (owner tx-sender)
      (device-data (unwrap! (map-get? devices { owner: owner, device-id: device-id }) ERR-DEVICE-NOT-FOUND))
      (current-activity (default-to { activity-count: u0, last-action: "" } (map-get? device-activity { device-id: device-id })))
    )
