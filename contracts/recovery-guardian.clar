...
;; Get guardian info
(define-read-only (get-guardian-info (owner principal) (guardian principal))
...
(define-read-only (get-account-guardians (owner principal))
  (default-to 
    { guardian-list: (list), guardian-threshold: u0, total-guardians: u0 }
...
(define-read-only (can-execute-recovery (owner principal))
  (match (map-get? recovery-requests { owner: owner })
    recovery-data
      (let
        (
          (guardian-config (unwrap! (map-get? account-guardians { owner: owner }) (err ERR-GUARDIAN-NOT-FOUND)))
          (threshold (get guardian-threshold guardian-config))
          (approvals (get approval-count recovery-data))
          (timelock-expired (>= (- stacks-block-height (get initiated-at recovery-data)) RECOVERY-TIMELOCK))
        )
        (ok (and 
...
;; Verify principal structure using principal-destruct? (Clarity 4)
(define-read-only (verify-guardian-principal (guardian principal))
  (match (principal-destruct? guardian)
    principal-data (ok true)
...
(define-private (is-caller-guardian (owner principal))
  (match (map-get? guardians { owner: owner, guardian: tx-sender })
...
(define-public (remove-guardian (guardian principal))
  (let
    (
      (owner tx-sender)
      (guardian-data (unwrap! (map-get? guardians { owner: owner, guardian: guardian }) ERR-GUARDIAN-NOT-FOUND))
      (current-guardians (get-account-guardians owner))
    )
...
(define-public (set-guardian-threshold (threshold uint))
  (let
    ...
    ;; Validate threshold
    (asserts! (and (>= threshold u1) (<= threshold total-guardians)) ERR-INVALID-THRESHOLD)
...
(define-public (initiate-recovery (account-owner principal) (new-owner principal))
  (let
    (
      (guardian-data (unwrap! (map-get? guardians { owner: account-owner, guardian: tx-sender }) ERR-NOT-AUTHORIZED))
    )
...
(define-public (approve-recovery (account-owner principal))
  (let
    (
      (guardian-data (unwrap! (map-get? guardians { owner: account-owner, guardian: tx-sender }) ERR-NOT-AUTHORIZED))
      (recovery-data (unwrap! (map-get? recovery-requests { owner: account-owner }) ERR-RECOVERY-NOT-ACTIVE))
    ...
    (map-set guardians
      { owner: account-owner, guardian: tx-sender }
...
(define-public (execute-recovery (account-owner principal))
  (let
    (
      (recovery-data (unwrap! (map-get? recovery-requests { owner: account-owner }) ERR-RECOVERY-NOT-ACTIVE))
      (guardian-config (unwrap! (map-get? account-guardians { owner: account-owner }) ERR-GUARDIAN-NOT-FOUND))
    ...
    (map-set recovery-requests
      { owner: account-owner }
      (merge recovery-data { executed: true, is-active: false })
    )
...
(define-public (cancel-recovery)
  (let
    (
      (owner tx-sender)
      (recovery-data (unwrap! (map-get? recovery-requests { owner: owner }) ERR-RECOVERY-NOT-ACTIVE))
    )
