...
(define-read-only (get-balance (owner principal))
  (ok (get balance (default-to { balance: u0, nonce: u0, multisig-threshold: u1, total-passkeys: u0, created-at: u0 }
    (map-get? wallets { owner: owner }))))
)
...
(define-read-only (has-enough-approvals (tx-id uint))
  (match (map-get? pending-transactions { tx-id: tx-id })
    tx-data
      (let
        (
          (wallet-info (unwrap! (map-get? wallets { owner: (get from tx-data) }) (err ERR-WALLET-NOT-FOUND)))
...
(define-private (verify-passkey-auth (owner principal) (passkey-id (buff 65)))
...
(define-public (initialize-wallet (initial-threshold uint))
  (let
    ...
    ;; Validate threshold
    (asserts! (and (>= initial-threshold u1) (<= initial-threshold u10)) ERR-INVALID-THRESHOLD)
...
(define-public (set-multisig-threshold (new-threshold uint))
  (let
    ...
    ;; Validate threshold
    (asserts! (and (>= new-threshold u1) (<= new-threshold (get total-passkeys wallet-data))) ERR-INVALID-THRESHOLD)
